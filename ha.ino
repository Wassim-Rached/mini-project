...
<html lang='fr-FR'>
  <head>
    ...
    <script type='module'>
      ...
      const led_state__span = document.getElementById('led_state');

      function toggle_led_on() {
        const url = window.location.hostname + '/?GPIO2=1';
        fetch(url)
          .then((response) => {
        ...
        led_state__span.innerHTML = 'ON';
      }
      function toggle_led_off() {
        const url = window.location.hostname + '/?GPIO2=0';
        fetch(url)
          .then((response) => {
            return response.json();
          })
    ...
    <div id='secured_page'>
      ...
      <form method='GET' id='myForm' name='myForm'>
        <div id='command-etat'>
          ...
          <div id='etat'>GPIO2 : <span id='led_state'> </span></div>
        </div>
        <div onClick='toggle_led_on()'>
          <span> ON</span>
        </div>
        <div onClick='toggle_led_off()'><span>OFF</span></div>
      </form>
      <br /><br />
...
void Racine()
{
    if (server.method() == HTTP_POST && server.hasArg("GPIO2"))
    {
        handle_toogle_led();
    }
    else
